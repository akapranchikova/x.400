...
const specPath = path.resolve(fileURLToPath(new URL('.', import.meta.url)), '../../core-service/api/openapi.json');
...
describe('core-service OpenAPI contract', () => {
  it('parses and validates the contract document', async () => {
    ...
    expect(api.components?.schemas).toHaveProperty('MessageEnvelope');
  });
