...
export function makeReport(overrides: PartialDeep<Report> = {}): Report {
  const merged = deepMerge({ ...baseReport, id: randomUUID(), timestamp: new Date().toISOString() }, overrides);
...
export function makeMessage(overrides: PartialDeep<Message> = {}): Message {
  ...
  const reports =
    overrides.reports?.map((report) => makeReport({ messageId: envelope.id, ...report })) ?? [
      makeReport({ messageId: envelope.id }),
      makeReport({ messageId: envelope.id, type: 'read', timestamp: addMinutes(new Date(), 2).toISOString() })
    ];
