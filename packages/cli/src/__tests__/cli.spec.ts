...
const createTransportMock = () => {
  ...
  const transport = {
    connect: vi.fn().mockResolvedValue({ sessionId: 'test', connectedAt: new Date().toISOString(), peer: 'mock' }),
    folders: {
...
describe('CLI program', () => {
  ...
  it('retrieves a message summary via the message command', async () => {
    ...
    await program.parseAsync(['node', 'cli', 'message', '--id', message.envelope.id], { from: 'user' });
  ...
  it('parses addresses for create command and forwards to compose', async () => {
    ...
    await program.parseAsync(
      ['node', 'cli', 'create', '--from', 'C=DE;O=Modern;S=Operator', '--to', 'C=DE;O=Unit;S=One', '--subject', 'CLI Test'],
      { from: 'user' }
    );
  ...
  it('wait command polls until the queue is empty', async () => {
    ...
    vi.useFakeTimers();
    const execution = program.parseAsync(['node', 'cli', 'wait', '--timeout', '1'], { from: 'user' });
